...
def add_item_popularity(df: pd.DataFrame) -> pd.DataFrame:
    popularity = df.groupby("product_id").size().rename("purchase_count").reset_index()
...
def add_item_popularity_state(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.merge(pop_state, on=["customer_state", "product_id"], how="left")


def compute_cooccurrence(df: pd.DataFrame) -> pd.DataFrame:
    items_per_order = df.groupby("order_id")["product_id"].apply(list)
