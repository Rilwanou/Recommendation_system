...
def generate_negative_samples(
    ...
    user_bought = (
        positives.groupby("customer_unique_id")["product_id"]
        .apply(set)
        .to_dict()
    )
    ...
    negatives_key = pd.DataFrame(
        neg_rows,
        columns=["customer_unique_id", "product_id"]
    )
    ...
    negatives = (
        negatives_key
        .merge(user_meta, on="customer_unique_id", how="left")
    ...
    if "purchase_count_state" in positives.columns:
        negatives = negatives.merge(
            positives[["customer_state", "product_id", "purchase_count_state"]]
            .drop_duplicates(),
