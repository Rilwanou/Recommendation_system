...
def plot_top_categories(data, n=15):
    ...
    return px.bar(
        product, x="category", y="count", title=f"Top {n} catégories produits"
    )
...
def plot_top_cities(data, n=15):
    ...
    return px.bar(
        city, x="city", y="count", title=f"Répartition des clients par ville (Top {n})"
    )


def plot_length_vs_price(data):
    intermediate = data[["product_length_cm", "price"]].dropna().copy()
    intermediate["product_length_cm"] = pd.to_numeric(
        intermediate["product_length_cm"], errors="coerce"
    )
    ...
    return px.scatter(
        ...
        labels={"product_length_cm": "Longueur du produit (cm)", "price": "Prix"},
    )
