...
def recommend_items(model, customer_profile, products_df, top_k=5):
    ...
    product_categories = candidates_df['product_category_name_english'].copy() if 'product_category_name_english' in candidates_df.columns else None
...
def get_customer_statistics(df, customer_unique_id):
    ...
    stats = {
        ...
        'total_spent': customer_data['price'].sum() if 'price' in customer_data.columns else 0,
        'avg_price': customer_data['price'].mean() if 'price' in customer_data.columns else 0,
        'favorite_category': customer_data['product_category_name_english'].mode()[0] 
            if 'product_category_name_english' in customer_data.columns and len(customer_data) > 0 else "N/A",
        'recent_purchases': customer_data['recency_days'].min() if 'recency_days' in customer_data.columns else None,
        'conversion_rate': customer_data['label'].mean() if 'label' in customer_data.columns else None
    }
